...
| ID инструмента (nUR_MCP_SERVER) | Категория         | Описание                          | Параметры          |  
|--------------------------------|------------------|----------------------------------|--------------------|  
| ffWzqSZlUpjFfVITKvBb-b         | Управление подключением | Подключиться к роботу UR       | `ip`: IP робота   |  
| fx5l9Mb_BUs_hClarGKZIo         | Управление подключением | Отключиться от робота UR      | `ip`: IP робота   |  
| f04vj-Fwbuo8s2oYu252pR         | Мониторинг состояния | Получить время работы робота  | `ip`: IP робота   |  
| f2SHZtIYF_OhS8LuslKt8D         | Операции с регистрами | Чтение значения регистра Int (0-23) | `ip`: IP робота, `index`: Индекс регистра (0-23) |  
| fjLf_89qRxYDlEKo_oXADQ         | Операции с регистрами | Чтение значения регистра Double (0-23) | `ip`: IP робота, `index`: Индекс регистра (0-23) |  
| f9uXSEQB1UZMXmgIyPtVH9         | Операции с регистрами | Чтение значения регистра Double (0-31) | `ip`: IP робота, `index`: Индекс регистра (0-31) |  
| fBHQbdS9p_BPv97XBAau8a         | Информация об устройстве | Получить серийный номер робота | `ip`: IP робота   |  
| fcHIfX-J83SvH0V6XZ59A7         | Управление движением | Получить TCP-координаты в реальном времени | `ip`: IP робота   |  
| fwloSfxAGiC_4rjdaZjUng         | Управление движением | Получить углы сочленений в реальном времени | `ip`: IP робота   |  
| fFg2TP6jf3XTWytLIf9fyO         | Управление движением | Отправить команду положения сочленений | `ip`: IP робота, `q`: Углы сочленений (радианы), `a/v/t/r`: Параметры движения |  
| fBevTU5BWNdVNEKgqLV5d1         | Управление движением | Отправить команду линейного перемещения TCP | `ip`: IP робота, `pose`: Позиция TCP, `a/v/t/r`: Параметры движения |  
| fnPhKVsDYRrB_Cf1wlsitI         | Управление движением | Линейное перемещение по оси X    | `ip`: IP робота, `distance`: Дистанция перемещения (метры) |  
| fO55CY6Jw_iASo5NkxTm3l         | Управление движением | Линейное перемещение по оси Y    | `ip`: IP робота, `distance`: Дистанция перемещения (метры) |  
| fs3ppclCfK_x0ZzFaOKHMk         | Управление движением | Линейное перемещение по оси Z    | `ip`: IP робота, `distance`: Дистанция перемещения (метры) |  
| f18oyor3ddLJHMdPpyer45         | Управление программой | Загрузить программу UR           | `ip`: IP робота, `program_name`: Имя программы |  
| fQyW6AYZjmMiGL0unOVK0W         | Управление программой | Загрузить и выполнить программу UR | `ip`: IP робота, `program_name`: Имя программы |  
| fDxcON3qGCkzR5n9jSS5vQ         | Управление программой | Остановить текущую программу     | `ip`: IP робота   |  
| fXB2Q94gpe30w1JHgZh_yr         | Управление программой | Приостановить текущую программу  | `ip`: IP робота   |  
| fyXhJEun99j0N-5ojAzPYu         | Мониторинг состояния | Получить текущее напряжение      | `ip`: IP робота   |  
| fYvUCtj-7VJIdbm1FA6cg8         | Мониторинг состояния | Получить текущий ток             | `ip`: IP робота   |  
| fno_OQmW5X_c3HK8M3r_NV         | Мониторинг состояния | Получить напряжение сочленений   | `ip`: IP робота   |  
| fPc54VdscRk2hmlEc3VwYg         | Мониторинг состояния | Получить ток сочленений          | `ip`: IP робота   |  
| fAKuhJVkwgI4NbTKQ-xsoM         | Мониторинг состояния | Получить температуру сочленений  | `ip`: IP робота   |  
| fjZheWqG9gz8SO78UgN6v7         | Мониторинг состояния | Получить статус работы           | `ip`: IP робота   |  
| f2XEU4pWT2dsXI2rMsGfEw         | Мониторинг состояния | Получить статус выполнения программы | `ip`: IP робота |  
| fYLTKblGKQserA-ZvLXCAt         | Информация об устройстве | Получить версию ПО           | `ip`: IP робота   |  
| f2ebXFoN8bP3Pm1zocshdN         | Информация об устройстве | Получить режим безопасности    | `ip`: IP робота   |  
| f_5wAr6iv97d1jurrEmhNp         | Управление программой | Отправить скрипт-программу       | `ip`: IP робота, `script`: Содержимое скрипта |  
| fI3ZhZJZafBt33eNGD0ydQ         | Управление движением | Круговое движение                 | `ip`: IP робота, `center`: TCP-позиция центра, `r`: Радиус (метры), `coordinate`: Тип плоскости |  
